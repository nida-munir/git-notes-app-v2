...
import { updateGists, deleteGist, createGist, updateIsLoading, editGist } from '../../action-creators/index';
...
class NotebookList extends React.Component<NotebookProps, {}> {
	...
	columns = [
		{
			...
			key: 'description',
			render: (text: string, record: any) => <a onClick={() => this.handleNameClick(record)}>{text}</a>
		},
		{
			title: 'Files Count',
	...
	public render() {
		...
		const { showModal, handleGistCreation, handleGistCancel, handleInputChange, columns } = this;
		...
		return (
			<div>
				<Spin spinning={isLoading}>
					...
					<Table className="your-table" columns={columns} dataSource={gists} rowKey="id" />,
				</Spin>
			</div>
		);
	}
...
>;
type NoteBookStateProps = Pick<NotebookProps, 'gists' | 'history' | 'isLoading'>;

function mapStateToProps(state: ApplicationState, ownProps: any): NoteBookStateProps {
